...
規則：
- items 只放**一般股票或 ETF**，symbol 為純數字台股代碼（不加 .TW 後綴，原樣保留前綴 0），合法格式：
    • 4 位數一般股票（例「2330」「1711」）
...
**重要：ETF（00 開頭）必為 items，不要當成權證跳過。**

**務必放進 skippedWarrants 而非 items 的項目：**
- 權證：代號含英文字母（例「08501U」「739445P」），或名稱含「購」「售」「認購」「認售」「牛」「熊」等字樣
- 6 位純數字但**不是 00 開頭**的代號（例「739445」，常見為權證）
